...
def kb_tipo_selector(ctx):
    c=ctx.encode()
    volta=b"m_origens" if ctx in ("src","src_ign","src_rem") else (b"m_destinos" if ctx in ("dst","dst_rem") else b"disc_menu")
    return [
...
def kb_lista_chats(items,ctx,cat,pagina=0,por_pagina=8):
    ...
    for item in pagina_items:
        dado="{ctx}|sel|{id}|{cat}".format(ctx=ctx,id=item["id"],cat=cat).encode()
        linhas.append([Button.inline(item["name"][:48],dado)])
    nav=[]
    if pagina>0:
        nav.append(Button.inline("<< Anterior","{ctx}|pg|{cat}|{p}".format(ctx=ctx,cat=cat,p=pagina-1).encode()))
    if inicio+por_pagina<len(items):
        nav.append(Button.inline("Proxima >>","{ctx}|pg|{cat}|{p}".format(ctx=ctx,cat=cat,p=pagina+1).encode()))
    if nav: linhas.append(nav)
    linhas.append([Button.inline("-- Manual --","{ctx}|manual".format(ctx=ctx).encode()),
                   Button.inline("<< Voltar","{ctx}|back".format(ctx=ctx).encode())])
    return linhas

def kb_origens():
    return [
        [Button.inline("+ Adicionar origem",   b"src|tipo"),
         Button.inline("- Remover origem",     b"src_rem|tipo")],
        [Button.inline("x Ignorar chat",       b"src_ign|tipo"),
...
def kb_destinos():
    return [
        [Button.inline("+ Adicionar destino",   b"dst|tipo"),
         Button.inline("- Remover destino",     b"dst_rem|tipo")],
...
def kb_agenda():
    ...
    return [
        [Button.inline("Definir horario",         b"ag_set"),
         Button.inline("Agendamento: "+ativo,     b"ag_toggle")],
...
def kb_tipos():
    ...
    for i in range(0,len(tipos),3):
        ...
        for t in tipos[i:i+3]:
            ...
            linha.append(Button.inline(ativo+" "+t,"tp_"+t))
        linhas.append(linha)
...
def status_texto():
    ...
    lines=[
        ...
        "Ignorados: "+str(IGNORADOS or "nenhum"),
        sep,
...
async def cmd_stats(ev):
    ...
    t="Envios por hora:\n"
    t+="".join("  "+str(h).zfill(2)+"h: "+("|"*min(n,20))+" "+str(n)+"\n" for h,n in horas.items())
    t+="\nTotal: "+str(stats["n"])+"  |  Erros: "+str(stats["err"])
    await ev.respond(t)
...
async def entrada_usuario(ev):
    ...
    if acao=="src|manual":
        ...
        await ev.respond("Origens adicionadas: "+str(ids)+"\nAtivas: "+str(SRC or "todos"),buttons=kb_origens())
    elif acao=="src_rem|manual":
        ids=parse_ids(txt)
        for i in ids: SRC.discard(int(i))
        await ev.respond("Removidas: "+str(ids)+"\nAtivas: "+str(SRC or "todos"),buttons=kb_origens())
        ...
        await ev.respond("Destinos adicionados: "+str(ids)+"\nAtivos: "+str(DESTINOS),buttons=kb_destinos())
    elif acao=="dst_rem|manual":
        ids=parse_ids(txt)
        for i in ids: DESTINOS.discard(int(i))
        await ev.respond("Removidos: "+str(ids)+"\nAtivos: "+str(DESTINOS),buttons=kb_destinos())
        ...
        try:
            ...
            resp="Resultado:\n"+"="*16+"\nTipo: "+tipo+"\nNome: "+nome+"\nID: "+str(eid)
            if uname: resp+="\nUsername: @"+uname
            await ev.respond(resp,buttons=[[Button.inline("Descobrir outro",b"disc_menu"),Button.inline("<< Menu",b"m_back")]])
        except Exception as e:
            await ev.respond("Nao encontrado: "+str(e)+"\nDigite /menu para voltar.")
    elif acao=="o_des":
        ids=parse_ids(txt)
        for i in ids: IGNORADOS.discard(int(i))
        await ev.respond("Designorado(s): "+str(ids),buttons=kb_origens())
    elif acao=="f_add_on":
        p=[x.lower() for x in txt.split()]; FILTROS_ON.update(p)
        await ev.respond("Exigidas: "+str(FILTROS_ON),buttons=kb_filtros())
    elif acao=="f_add_off":
        p=[x.lower() for x in txt.split()]; FILTROS_OFF.update(p)
        await ev.respond("Bloqueadas: "+str(FILTROS_OFF),buttons=kb_filtros())
    elif acao=="f_rem":
        p=[x.lower() for x in txt.split()]
        for x in p: FILTROS_ON.discard(x);FILTROS_OFF.discard(x)
        await ev.respond("Removido. +:"+str(FILTROS_ON)+" | -:"+str(FILTROS_OFF),buttons=kb_filtros())
    elif acao=="mg_prefix":
        PREFIXO=txt
        ...
        try:
            ...
            await ev.respond("Horario: "+AGENDAMENTO["inicio"]+" ate "+AGENDAMENTO["fim"],buttons=kb_agenda())
...
async def callback(ev):
    ...
    if d==b"m_back":
        ...
        if not HISTORICO:
            ...
            await ev.edit(t,buttons=[[Button.inline("<< Voltar",b"m_back")]])
    elif d==b"m_status":
        await ev.edit(status_texto(),buttons=[[Button.inline("Atualizar",b"m_status"),Button.inline("<< Voltar",b"m_back")]])
        ...
        label_map={"src":"ADICIONAR ORIGEM","src_rem":"REMOVER ORIGEM","src_ign":"IGNORAR CHAT","dst":"ADICIONAR DESTINO","dst_rem":"REMOVER DESTINO"}
        await ev.edit(label_map.get(ctx,"SELECIONAR")+"\n"+"="*20+"\nEscolha o tipo de chat:",buttons=kb_tipo_selector(ctx))

    elif b"|pg|" in d:
        partes=d.decode().split("|"); ctx,cat,pagina=partes[0],partes[2],int(partes[3])
        ...
        await ev.edit(cat.upper()+" -- "+str(len(items))+" encontrado(s)\nSelecione:",buttons=kb_lista_chats(items,ctx,cat,pagina))
        ...
        if sub=="back":
            if ctx in ("src","src_rem","src_ign"): await ev.edit("ORIGENS",buttons=kb_origens())
            elif ctx in ("dst","dst_rem"): await ev.edit("DESTINOS",buttons=kb_destinos())
            else: await ev.edit("DESCOBRIR ID",buttons=[[Button.inline("<< Menu",b"m_back")]])
            return
        ...
        if sub=="sel" and len(partes)>=4:
            ...
            if ctx=="src":
                SRC.add(chat_id)
                await ev.edit("ORIGEM ADICIONADA\n"+"="*20+"\n"+nome+"\nID: "+str(chat_id)+"\nOrigens ativas: "+str(len(SRC)),
                    buttons=[[Button.inline("+ Adicionar outra",b"src|tipo")],[Button.inline("= Ver origens",b"o_list")],[Button.inline("<< Origens",b"m_origens"),Button.inline("Home",b"m_back")]])
            elif ctx=="src_rem":
                SRC.discard(chat_id)
                await ev.edit("ORIGEM REMOVIDA\n"+"="*20+"\n"+nome+"\nRestantes: "+str(len(SRC)),
                    buttons=[[Button.inline("<< Origens",b"m_origens"),Button.inline("Home",b"m_back")]])
            elif ctx=="src_ign":
                IGNORADOS.add(chat_id)
                await ev.edit("CHAT IGNORADO\n"+"="*20+"\n"+nome+"\nID: "+str(chat_id),
                    buttons=[[Button.inline("<< Origens",b"m_origens"),Button.inline("Home",b"m_back")]])
            elif ctx=="dst":
                DESTINOS.add(chat_id)
                await ev.edit("DESTINO ADICIONADO\n"+"="*20+"\n"+nome+"\nID: "+str(chat_id)+"\nDestinos ativos: "+str(len(DESTINOS)),
                    buttons=[[Button.inline("+ Adicionar outro",b"dst|tipo")],[Button.inline("= Ver destinos",b"d_list")],[Button.inline("<< Destinos",b"m_destinos"),Button.inline("Home",b"m_back")]])
            elif ctx=="dst_rem":
                DESTINOS.discard(chat_id)
                await ev.edit("DESTINO REMOVIDO\n"+"="*20+"\n"+nome+"\nRestantes: "+str(len(DESTINOS)),
    ...
    elif d==b"o_des": AGUARDANDO[uid]="o_des"; await ev.answer("Ignorados: "+str(IGNORADOS)+"\nDigite IDs para designorar:",alert=True)
    elif d==b"o_list":
    ...
    elif d==b"mo_fwd": MOD="forward"; await ev.edit("MODO\nAtual: forward",buttons=kb_modo()); await ev.answer("Modo: forward",alert=True)
    elif d==b"mo_copy": MOD="copy"; await ev.edit("MODO\nAtual: copy",buttons=kb_modo()); await ev.answer("Modo: copy",alert=True)
    elif d==b"mo_bots":
        SEM_BOTS=not SEM_BOTS
        await ev.edit("MODO\nSem bots: "+("SIM" if SEM_BOTS else "NAO"),buttons=kb_modo())
        await ev.answer("Bots ignorados: "+("SIM" if SEM_BOTS else "NAO"),alert=True)
    elif d==b"mo_delay": AGUARDANDO[uid]="mo_delay"; await ev.answer("Delay atual: "+str(DELAY)+"s\nDigite em segundos:",alert=True)
    elif d==b"mo_tipos": await ev.edit("TIPOS DE MIDIA\n[x]=ativo  [ ]=inativo",buttons=kb_tipos())
    elif d==b"mo_tipos_back": await ev.edit("MODO",buttons=kb_modo())
        ...
        await ev.edit("TIPOS DE MIDIA",buttons=kb_tipos())
    ...
    elif d==b"f_rem": AGUARDANDO[uid]="f_rem"; await ev.answer("+:"+str(FILTROS_ON)+"\n-:"+str(FILTROS_OFF)+"\nPalavras para remover:",alert=True)
    elif d==b"f_list": await ev.answer("Exigidas: "+str(FILTROS_ON or "nenhuma")+"\nBloqueadas: "+str(FILTROS_OFF or "nenhuma"),alert=True)
        ...
        ativo="ATIVO" if AGENDAMENTO["ativo"] else "INATIVO"
        await ev.edit("HORARIO\nEstado: "+ativo+"\nJanela: "+AGENDAMENTO["inicio"]+" ate "+AGENDAMENTO["fim"],buttons=kb_agenda())
        await ev.answer("Agendamento "+ativo+"!",alert=True)
    elif d==b"ag_set": AGUARDANDO[uid]="ag_set"; await ev.answer("Formato: HH:MM HH:MM\nEx: 08:00 22:00",alert=True)
    elif d==b"ag_ver": await ev.answer(("ATIVO" if AGENDAMENTO["ativo"] else "INATIVO")+"\nInicio: "+AGENDAMENTO["inicio"]+"\nFim: "+AGENDAMENTO["fim"],alert=True)
    elif d==b"mg_prefix": AGUARDANDO[uid]="mg_prefix"; await ev.answer("Prefixo atual: "+(PREFIXO or "nenhum")+"\nNovo prefixo:",alert=True)
    elif d==b"mg_suffix": AGUARDANDO[uid]="mg_suffix"; await ev.answer("Rodape atual: "+(RODAPE or "nenhum")+"\nNovo rodape:",alert=True)
    ...
    elif d==b"mg_ver": await ev.answer("Prefixo: "+(PREFIXO or "nenhum")+"\nRodape: "+(RODAPE or "nenhum"),alert=True)
    elif d==b"i_ping":
        up=datetime.now()-stats["start"]; h2,r=divmod(int(up.total_seconds()),3600); mi,s=divmod(r,60)
        await ev.answer("Pong!\nUptime: "+str(h2)+"h "+str(mi)+"m "+str(s)+"s",alert=True)
    elif d==b"i_id": await ev.answer("ID deste chat: "+str(ev.chat_id),alert=True)
    elif d==b"i_stats":
        horas=dict(sorted(stats["por_hora"].items())[-8:])
        t="Por hora:\n"+"".join("  "+str(h2).zfill(2)+"h: "+str(n)+"\n" for h2,n in horas.items())
        t+="\nTotal: "+str(stats["n"])+" | Erros: "+str(stats["err"])
        await ev.answer(t,alert=True)
    elif d==b"i_reset":
        stats["n"]=0;stats["err"]=0;stats["por_hora"].clear();stats["start"]=datetime.now()
        await ev.answer("Stats zeradas!",alert=True)
    elif d==b"i_teste":
        if not DESTINOS: await ev.answer("Nenhum destino!",alert=True); return
        ok=0
        for dst in DESTINOS:
            try: await bot.send_message(dst,BOT_NOME+" -- Teste OK\n"+datetime.now().strftime("%d/%m %H:%M")); ok+=1
            except Exception as e: logger.error("Teste falhou em "+str(dst)+": "+str(e))
        await ev.answer("Teste enviado: "+str(ok)+"/"+str(len(DESTINOS))+" destino(s)!",alert=True)
    elif d==b"disc_menu":
        await ev.edit("DESCOBRIR ID\n"+"="*20+"\nEscolha o tipo ou busque pelo username:",buttons=[
        ...
        for item in items[:20]:
            ...
            linhas.append([Button.inline(item["name"]+uname,"disc_show|"+str(item["id"]))])
        linhas.append([Button.inline("-- Buscar manual --",b"disc|manual"),Button.inline("<< Voltar",b"disc_menu")])
    ...
    try:
        ...
        for dst in DESTINOS:
            try:
                ...
                ok+=1
            except Exception as e: stats["err"]+=1; logger.error("Erro ao enviar "+str(dst)+": "+str(e))
        if ok>0:
            stats["n"]+=1; stats["por_hora"][datetime.now().hour]+=1
            ...
            if not MODO_SILENCIOSO: logger.info("["+BOT_NOME+"] #"+str(stats["n"])+" de '"+name+"' -> "+str(ok)+" destino(s)")
    except Exception as e: stats["err"]+=1; logger.error("Erro geral: "+str(e))
...
async def main():
    ...
    logger.info("["+BOT_NOME+"] Userbot: "+me.first_name+" | Bot: @"+bme.username)
    logger.info("Destinos="+str(DESTINOS)+" | Origens="+str(SRC or "todos")+" | Modo="+MOD)
